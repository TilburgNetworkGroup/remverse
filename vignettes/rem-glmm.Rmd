...
where $u_i$ is the sender-specific random intercept. Random effects are requested via the `random` argument, which uses `lme4` formula syntax. We set `engine = "lme4"` for reproducibility (the default `"auto"` uses `glmmTMB` when installed).

```{r sender-frailty, message=FALSE, warning=FALSE}
...
```{r receiver-frailty, message=FALSE, warning=FALSE}
...
```{r crossed, message=FALSE, warning=FALSE}
fit_crossed <- remfrailty(reh = reh, stats = stats, engine = "lme4")
summary(fit_crossed)
```
...
```{r random-slope, message=FALSE, warning=FALSE}
