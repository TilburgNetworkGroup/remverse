...
# Step 2: specify statistics for sender and receiver models separately
...
reh_ao_ord <- remify(edgelist = history, model = "actor", ordinal = TRUE, riskset = "active")
...
```{r undirected}
reh_undir <- remify(edgelist = history, model = "tie", directed = FALSE)
...
- `send("extraversion")`: quantifies whether extraverted actors tend to initiate more events.
- `receive("agreeableness")`: quantifies whether agreeable actors tend to be chosen more often as receivers.
- `average("extraversion")`: quantifies the effect of the average extraversion of the actors in the dyad on the event rate.
...
```{r sampling}
reh_samp <- remify(edgelist = history, model = "tie", directed = TRUE, event_type = "setting")
...
Both tie-oriented and actor-oriented models can be estimated with Hamiltonian Monte Carlo via `method = "HMC"`. By default, independent normal priors with mean 0 and variance 100 are placed on all coefficients. Custom priors can be specified via the `prior` argument:

```{r hmc, message=FALSE}
P <- length(dimnames(stats)[[3]])
my_prior <- list(
  mean = rep(0, P),
